...
Fukidashi MCP is an offline, headless Rust 2024 server for local comic page analysis, cleaning, typesetting, editing, review, and export. It exposes eleven MCP tools, including the strict-v1 `fukidashi_translation_start`/`fukidashi_translation_submit` pair, read-only `fukidashi_get_config`, and persistent `fukidashi_configure` so each user can choose storage from any working directory. Supplied dialogue and translations are passed through verbatim.
...
For model clients that tend to wander through managed files, use the strict-v1 loop: call `fukidashi_translation_start` with exactly one `image_path`, `job_path`, or `job_id`, then submit the exact stable ID set through `fukidashi_translation_submit`. The submit request contains only `work_token` and `translations`; each item supplies `translation` or explicit `keep_source: true`, with `needs_review: true` preserving uncertainty through the render. The server owns page selection, analysis, clean, typeset, stage reuse, model release, and advancement. Tokens bind the current page and analysis hash, so stale or duplicate submissions fail closed. A final response returns `review_ready` and the exact `fukidashi_serve_editor` job argument; call `fukidashi_wait_for_review` immediately with the editor's session and revision. Never shell-read managed manifests/checkpoints or construct artifact paths. Pages with no translation items are reported as `needs_manual_scope`; the server does not fake an unchanged clean artifact.
