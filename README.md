...
Direct URL ingress uses an already provisioned gallery-dl executable discovered in FUKIDASHI_GALLERY_DL, storage_root/bin, or PATH; `fukidashi_get_config` and `doctor` report the selected helper. Native acquisition uses only the official MangaDex API, and all imported pages are validated and transactionally staged as files before a new managed job is published. Imported MangaDex jobs use a sanitized canonical manga title plus chapter prefix; direct imports accept an optional bounded `job_name` or derive a conservative host/path label.
...
`fukidashi_search_manga` searches native MangaDex titles only. `fukidashi_pull_chapter` resolves an exact MangaDex manga/chapter and official at-home page list, or accepts one explicit http(s) URL for an already installed gallery-dl helper. Downloads are bounded, validated, staged transactionally as owned files, and imported one page at a time into a fresh managed job; the response returns the exact `fukidashi_translation_start` call for that job. No aggregator search, guessed URL, runtime helper download, or live-job mutation is used.
