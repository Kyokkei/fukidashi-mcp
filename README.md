...
glyph cluster is absent from Comic Neue. See `assets/fonts/README.md` for upstream
provenance, SIL Open Font License notices, attribution, and hashes.

Direct URL ingress uses an already provisioned gallery-dl executable discovered in FUKIDASHI_GALLERY_DL, storage_root/bin, or PATH; `fukidashi_get_config` and `doctor` report the selected helper. Native acquisition uses only the official MangaDex API, and all imported pages are validated and transactionally staged before a new managed job is published.
