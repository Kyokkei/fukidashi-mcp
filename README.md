...
Fukidashi MCP is a headless Rust 2024 server for local comic page analysis, cleaning, typesetting, editing, review, and export. It exposes MCP tools including the strict-v1 `fukidashi_translation_start`/`fukidashi_translation_submit` pair and the single-call `fukidashi_review_and_export` review gate, read-only `fukidashi_get_config`, and persistent `fukidashi_configure` so each user can choose storage from any working directory. Supplied dialogue and translations are passed through verbatim.
...
Comic Neue Regular, Comic Neue Bold, Patrick Hand Regular, and Noto Sans
Symbols 2 Regular are bundled in the executable for typesetting. Their TTF
bytes are embedded at compile time, then copied atomically into each managed
job's `fonts/` directory, so the release works from any current directory
without a font download. Comic Neue Regular remains the default dialogue face;
fallback selection is per grapheme cluster, preserving the primary face for
ordinary Vietnamese text while using Noto Sans Symbols 2 only for symbols such
as `❤`. See `assets/fonts/README.md` for upstream provenance, SIL Open Font
License notices, attribution, and hashes.

Direct URL ingress is first-class: it accepts one explicit `http(s)` URL plus an optional bounded `job_name`, then uses an already provisioned gallery-dl executable discovered in FUKIDASHI_GALLERY_DL, storage_root/bin, or PATH; `fukidashi_get_config` and `doctor` report the selected helper. Native acquisition uses only the official MangaDex API. Search returns an exact manga_id and a suggested `latest: true` call; latest resolves the highest chapter value including external releases, reports external or unavailable releases with `imported: false`, and never silently falls back to an older hosted chapter. Source language is metadata that Fukidashi auto-translates. If gallery-dl cannot extract an explicit URL, the response says it was not imported and can be retried with another explicit supported `http(s)` URL. Imported pages are validated and transactionally staged as files before a new managed job is published. Imported MangaDex jobs use a sanitized canonical manga title plus chapter prefix; direct imports derive a conservative host/path label when no job_name is supplied.
